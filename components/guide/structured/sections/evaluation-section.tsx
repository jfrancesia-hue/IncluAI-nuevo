      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
        <div className="rounded-[16px] border border-[#e2e8f0] bg-white p-6 shadow-[0_2px_12px_rgba(15,34,64,0.06)]">
          <div className="mb-4 flex items-center gap-2">
            ...
            <p className="text-[11px] font-semibold uppercase tracking-wider text-[#4A5968]">
              Evaluación tradicional
            </p>
          </div>
          <ul className="flex flex-col gap-3 text-sm">
            {section.traditional.map((t, i) => (
              <li key={i} className="flex gap-2 text-[#1F2E3D]">
        ...
        <div className="relative rounded-[16px] border-2 border-[#27AE60] bg-[#D6F0E0]/30 p-6">
          {section.adaptedRibbon && (
            <span className="absolute -top-3 left-6 rounded-full bg-[#27AE60] px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-white">
              {section.adaptedRibbon}
            </span>
          )}
          <div className="mb-4 flex items-center gap-2">
            <span className="flex h-8 w-8 items-center justify-center rounded-full bg-[#27AE60] text-white">
              <IconCheck width={16} height={16} stroke="white" strokeWidth={2.5} />
            </span>
            <p className="text-[11px] font-semibold uppercase tracking-wider text-[#27AE60]">
              Evaluación adaptada
            </p>
          </div>
          <ul className="flex flex-col gap-3 text-sm">
            {section.adapted.map((a, i) => (
              <li key={i} className="flex gap-2 text-[#1F2E3D]">
                <span className="font-bold text-[#27AE60]">{i + 1}.</span>
                <span>{a}</span>
              </li>
            ))}
          </ul>
  ...
  return (
    <div className="mt-6 rounded-[16px] border border-[#e2e8f0] bg-white p-6 shadow-[0_2px_12px_rgba(15,34,64,0.06)]">
      <div className="mb-4 flex items-center justify-between">
        ...
        <span className="text-xs text-[#4A5968]">{rubric.scaleNote ?? 'escala 0-4'}</span>
      </div>
  ...
  return (
    <div>
      <div className="mb-1.5 flex justify-between text-sm">
        <span className="text-[#2E86C1]">{item.criterion}</span>
