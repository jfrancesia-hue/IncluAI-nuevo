  ...
  return (
    <aside aria-label="Índice de la guía" className="order-2 lg:order-1">
      <div className="sticky top-24">
        <p className="mb-3 text-[11px] font-semibold uppercase tracking-wider text-[#4A5968]">
        ...
        <div className="mb-5 flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-[#27AE60]">{progress}%</span>
        </div>
            ...
            return (
              <a
                ...
                className={`flex items-center gap-3 rounded-[10px] px-3 py-2.5 text-sm transition ${
                  isActive
                    ? 'bg-[#D6F0E0] font-semibold text-[#27AE60]'
                    : isDone
                      ? 'text-[#2E86C1] hover:bg-white'
                      : 'text-[#4A5968] hover:bg-white hover:text-[#2E86C1]'
                }`}
              >
                <span
                  className={`h-2.5 w-2.5 shrink-0 rounded-full ${
                    isActive
                      ? 'bg-[#27AE60] ring-4 ring-[#D6F0E0]'
                      : isDone
                        ? 'bg-[#27AE60]'
                        : 'bg-[#cbd5e1]'
                  }`}
        ...
        <div className="mt-8 rounded-[14px] border border-[#e2e8f0] bg-white p-4 shadow-[0_2px_12px_rgba(15,34,64,0.06)]">
          ...
          <a
            href="#refinar"
            className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-[10px] bg-[#E67E22] px-3 py-2 text-xs font-bold text-white hover:bg-[#9a3412]"
          >
