...
import { SYSTEM_PROMPT_PROFESIONALES, buildPromptProfesionales } from '@/lib/prompts';
import { formularioProfesionalSchema } from '@/lib/validators';
...
export async function POST(request: NextRequest) {
  ...
  const parsed = formularioProfesionalSchema.safeParse(payload);
  if (!parsed.success) {
  ...
  return streamGuiaYResponder({
    ...
    userPrompt: buildPromptProfesionales(form),
